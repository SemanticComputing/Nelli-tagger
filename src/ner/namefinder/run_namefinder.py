...
class RunNameFinder(RunTool):
    ...
    def run(self, multiprocess=False, context=0):
        ...
        if multiprocess:
            ...
            logger.debug('Items: %s (%s)', items, chunk_dict)
            ...
            contexts = [ context for i in range(0, len(items), chunksize)]

            logger.debug('Multiprocessing chunks: %s',chunks)
            logger.info('Multiprocessing with params: %s', contexts)

            files = pool.map(self.execute_tool, chunks, contexts)
            ...
            start = time.time()
            items = list(self.input_texts.items())
            results = self.execute_tool(items, context)
            self.output_files = results
            end = time.time()
            logger.info("[STATUS] Executed Finer queries using single input in %s",
                        str(datetime.timedelta(seconds=(end - start))))

    def execute_tool(self, data, context=0):

        logger.info("[STATUS] Start a worker for data %s and params%s", data, context)
        ...
        try:
            for tpl in data:
                if len(tpl[1]) > 1:
                    ...
                    if context > 0:
                        if context > 100:
                            if int(str(context)[0]) > 1:
                                params['gender'] = True
                            #else:
                            if int(str(context)[1]) > 1:
                                params['title'] = True
                            #else:
                            if int(str(context)[2]) > 1:
                                params['date'] = True
                        else:
                            params['gender'] = True
                            params['title'] = True
                    ...
                    logger.info("Shoot %s, %s", str(url), str(params))
                    start = time.time()
                    ...
                    if resp.status_code == 200:
                        ...
                        logger.warning('Error %s for query %s, params %s', resp.status_code, url, params)
